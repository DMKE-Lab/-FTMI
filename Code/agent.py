...
class Agent(object):
    ...
    def __init__(self, params):
        ...
        self.dummy_start_relation = tf.constant(
            np.ones(self.batch_size, dtype='int64') * params['relation_vocab']['DUMMY_START_RELATION'])
        self.dummy_start_tim = tf.constant(
            np.ones(self.batch_size, dtype='int64') * params['tim_vocab']['DUMMY_START_TIM'])
        #self.dummy_start_label = np.array([self.dummy_start_relation, self.dummy_start_tim])
        ...
        with tf.variable_scope("relation_lookup_table"):
            self.relation_embedding_placeholder = tf.placeholder(tf.float32,
                                                               [self.relation_vocab_size, 2 * self.embedding_size])
        ...
        with tf.variable_scope("tim_lookup_table"):
            self.tim_embedding_placeholder = tf.placeholder(tf.float32,
                                                               [self.tim_vocab_size, 2 * self.embedding_size])
    ...
    def policy_MLP(self, state):
        with tf.variable_scope("MLP_for_policy"):
            ...
            hidden = tf.layers.dense(state, 4 * self.hidden_size, activation=tf.nn.leaky_relu)

            output = tf.layers.dense(hidden, self.m * self.embedding_size, activation=tf.nn.leaky_relu)
    ...
    def action_encoder(self, next_relations, next_entities):
        with tf.variable_scope("lookup_table_edge_encoder"):
            ...
            #relation_embedding = relation_embedding + tim_embedding
            if self.use_entity_embeddings:
    ...
    def step(self, next_relations, next_tims, next_entities, prev_state, prev_relation, prev_tim, query_embedding, query_embedding_tim, current_entities,
        ...
        prev_relation_embedding = prev_relation_embedding + prev_tim_embedding
        #prev_action_embedding = tf.concat([prev_relation_embedding, prev_tim_embedding], axis=-1) #3
        #prev_action_embedding = np.array2string(prev_action_embedding)

        # 1. one step of rnn
        output, new_state = self.policy_step(prev_relation_embedding, prev_state)  # output: [B, 4D]
        ...
        query_embedding1 = query_embedding + query_embedding_tim
        state_query_concat = tf.concat([state, query_embedding1], axis=-1)
        ...
        #output_tim = self.policy_MLPs(state_query_concat_tim)
        output_expanded = tf.expand_dims(output, axis=1)  # [B, 1, 2D]

        #output_expanded_tim = tf.expand_dims(output_tim, axis=1)
        ...
        dummy_scores_tim = tf.ones_like(prelim_scores_tim) * -99999.0
        scores_tim = tf.where(mask_tim, dummy_scores_tim, prelim_scores_tim)

        #q = tf.concat([scores, scores_tim], axis=1 )
        #scores1 = tf.reshape(q, [-1, 200])
        #qq = scores1.shape
        ...
        #qq = tf.concat([action, tim], axis=1  )
        #label_action1 = tf.reshape(qq, [-1, 1] )
        #label_action2 = tf.squeeze(label_action1, axis=1)

        loss1 = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=scores+scores_tim, labels=label_action)  # [B,]
        loss2 = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=scores_tim, labels=label_tim)
        loss = tf.add(loss1, loss2)
        #qqq = loss.shape
        #with tf.Session() as sess:
            #print(sess.run(qqq))
        #loss_tim = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=scores_tim, labels=label_tim)
        #loss = ([loss_action, loss_tim])
    ...
    def __call__(self, candidate_relation_sequence, candidate_tim_sequence, candidate_entity_sequence, current_entities,
        ...
        #query_embedding = np.array([query_embedding_relation, query_embedding_tim])
