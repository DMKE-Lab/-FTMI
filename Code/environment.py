...
class Episode(object):
    ...
    def get_reward(self,aaa):
        ...
        reward1 = np.select(condlist, choicelist)  # [B,]
        #else:
        ...
        #self.upscales = np.max(up, axis=1)
        reward2 = 1 / (1 + np.exp(- aaa/self.query_relation))
        reward2 = reward2 * (1 - reward2)



        reward = reward1+(1-reward1)* reward2
